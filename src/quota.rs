...
fn build_limits(api_keys: &[ApiKeyConfig], quotas: &QuotaConfig) -> HashMap<String, ResolvedLimits> {
    api_keys
        .iter()
        .map(|key_config| {
...
impl QuotaManager {
    ...
    #[cfg(feature = "db")]
    pub fn new(api_keys: &[ApiKeyConfig], quotas: &QuotaConfig, database: Option<Database>) -> Self {
        Self {
    ...
    pub async fn check_quota_hashed(&self, key_hash: &str) -> QuotaCheckResult {
        ...
        // Slow path: need write lock to initialize or reset periods
        let mut usage_map = self.inner.usage.write().await;
        let key_hash_owned = key_hash.to_string();
        let usage = usage_map.entry(key_hash_owned.clone()).or_insert_with(|| KeyUsage {
            daily: PeriodUsage {
                total_tokens: 0,
                period_start: today,
            },
        ...
        #[cfg(feature = "db")]
        Self::reset_periods_if_needed(usage, &key_hash_owned, today, this_month_start, &self.inner.database).await;
        #[cfg(not(feature = "db"))]
        Self::reset_periods_if_needed(usage, today, this_month_start);

        Self::evaluate_limits(&limits, usage)
    }
    ...
    pub fn check_quota_sync(&self, api_key: &str) -> Option<QuotaCheckResult> {
        ...
        let zero_usage = KeyUsage {
            daily: PeriodUsage { total_tokens: 0, period_start: today },
            monthly: PeriodUsage { total_tokens: 0, period_start: this_month_start },
        };
        ...
        let effective_usage = KeyUsage {
            daily: if usage.daily.period_start < today {
                PeriodUsage { total_tokens: 0, period_start: today }
            } else {
                usage.daily
            },
            monthly: if usage.monthly.period_start < this_month_start {
                PeriodUsage { total_tokens: 0, period_start: this_month_start }
            } else {
                usage.monthly
            },
        };
    ...
    /// Reset usage periods if they've rolled over, re-deriving from the database if available.
    #[cfg(feature = "db")]
    async fn reset_periods_if_needed(
        usage: &mut KeyUsage,
        key_hash: &str,
        today: NaiveDate,
        ...
        if let Some(db) = database {
            if let Ok((daily_total, monthly_total)) = db.load_quota_baseline_for_key(key_hash).await {
    ...
    fn evaluate_limits(limits: &ResolvedLimits, usage: &KeyUsage) -> QuotaCheckResult {
        ...
        QuotaCheckResult::Allowed {
            daily_remaining: limits.daily.map(|l| l.saturating_sub(usage.daily.total_tokens)),
    ...
    pub async fn record_usage(&self, api_key: &str, tokens: &TokenCounts) {
        self.record_usage_hashed(&hash_api_key(api_key), tokens).await;
    }
...
fn seconds_until_next_month() -> u64 {
    ...
    let next_month_local = next_month.and_hms_opt(0, 0, 0).unwrap().and_local_timezone(now.timezone()).unwrap();
...
fn next_day_timestamp() -> i64 {
    ...
    let tomorrow = (now.date_naive() + chrono::Duration::days(1))
        .and_hms_opt(0, 0, 0)
        .unwrap();
    tomorrow.and_local_timezone(now.timezone()).unwrap().timestamp()
}
...
fn next_month_timestamp() -> i64 {
    let now = Local::now();
    let next_month = next_month_start(now.date_naive());
    next_month.and_hms_opt(0, 0, 0).unwrap().and_local_timezone(now.timezone()).unwrap().timestamp()
}
...
mod tests {
    ...
    async fn test_quota_allowed_when_under_limit() {
        ...
        qm.record_usage("test-key", &TokenCounts { input: 100, output: 100, cache_read: 0, cache_write: 0 }).await;
    ...
    async fn test_quota_exceeded_daily() {
        ...
        qm.record_usage("test-key", &TokenCounts { input: 300, output: 200, cache_read: 0, cache_write: 0 }).await; // 500 total = at limit
    ...
    async fn test_quota_exceeded_monthly() {
        ...
        qm.record_usage("test-key", &TokenCounts { input: 600, output: 500, cache_read: 0, cache_write: 0 }).await; // 1100 > 1000
    ...
    async fn test_quota_unlimited_when_no_limits() {
        ...
        qm.record_usage("test-key", &TokenCounts { input: 999999, output: 999999, cache_read: 0, cache_write: 0 }).await;

        match qm.check_quota("test-key").await {
            QuotaCheckResult::Allowed { daily_remaining, monthly_remaining, .. } => {
    ...
    async fn test_per_key_override() {
        ...
        qm.record_usage("limited-key", &TokenCounts { input: 50, output: 60, cache_read: 0, cache_write: 0 }).await; // 110 > 100
        ...
        qm.record_usage("unlimited-key", &TokenCounts { input: 50, output: 60, cache_read: 0, cache_write: 0 }).await;
        match qm.check_quota("unlimited-key").await {
            QuotaCheckResult::Allowed { daily_remaining, .. } => {
    ...
    async fn test_cache_tokens_counted() {
        ...
        qm.record_usage("test-key", &TokenCounts { input: 100, output: 100, cache_read: 200, cache_write: 100 }).await;
    ...
    async fn test_zero_means_unlimited() {
        ...
        qm.record_usage("admin-key", &TokenCounts { input: 5000, output: 5000, cache_read: 0, cache_write: 0 }).await;
    ...
    async fn test_query_usage_with_data() {
        ...
        let rows = db.query_usage(Some(&key_hash), &today, crate::database::GroupBy::Day).await.unwrap();
    ...
    async fn test_cleanup_old_requests() {
        ...
        let today = format!("{} 00:00:00", Local::now().date_naive());
        let rows = db.query_usage(None, &today, crate::database::GroupBy::Day).await.unwrap();
