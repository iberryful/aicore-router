...
- **OpenAI Responses API (Codex CLI v0.130+).** `POST /v1/responses` is a transparent passthrough — the request body is forwarded unmodified to AI Core's native Responses endpoint. Token usage is read from the Responses-specific `usage.input_tokens` / `usage.output_tokens` / `usage.input_tokens_details.cached_tokens` shape (different field names from Chat Completions). Streaming events flow through verbatim; usage is recorded from any terminal frame — `response.completed`, `response.incomplete` (e.g., `max_output_tokens` reached), or `response.failed` (upstream error) — so partial-stream token counts still hit the quota and DB log.
